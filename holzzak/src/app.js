...
const roomsCollection = "oddEvenRooms";
...
function subscribe(roomCode) {
  ...
  state.unsubscribe?.();
  state.unsubscribe = onSnapshot(roomRef(roomCode), (snapshot) => {
    if (!snapshot.exists()) {
...
async function maybeFinishRound() {
  const snapshot = await getDoc(roomRef(state.roomCode));
...
async function finishRound(room, reason) {
  await runTransaction(db, async (transaction) => {
    const ref = roomRef(room.roomCode);
...
function roomRef(roomCode) {
  return doc(db, roomsCollection, normalizeRoomCode(roomCode));
}
