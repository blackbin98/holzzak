...
function render() {
  ...
  els.resultModal.classList.toggle("hidden", !["roundResult", "gameOver", "rematchStarting", "closing"].includes(room.status));
...
function playingNotice(room, myId) {
  ...
  if (hostChoice === undefined) {
    return myId === room.hostId
      ? "당신 차례예요. 30초 안에 상대를 흔들 카드 하나를 고르세요."
      : `${hostName}님이 카드를 고르는 중이에요. 30초 안에 선택하지 못하면 당신이 이깁니다.`;
  }
  return myId === room.guesserId
    ? `${hostName}님이 카드를 골랐습니다. 이제 당신에게 30초가 주어졌어요. 홀인지 짝인지 선택하세요.`
    : `${guesserName}님이 고민 중이에요. 30초 안에 고르지 못하면 당신이 이깁니다.`;
}

function choiceStatus(room, myId) {
  if (room.choices?.[myId] !== undefined) return "선택 완료. 이제 상대의 판단을 기다리면 됩니다.";
  if (myId === room.hostId) return "카드를 선택하면 상대에게 새 30초와 홀/짝 버튼이 열립니다.";
