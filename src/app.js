...
const state = {
  ...
  musicTimer: null,
  musicMode: "normal",
  muted: localStorage.getItem("holzzakMuted") === "1",
  lastSoundKey: "",
  lastWarningSecond: 0,
  celebratedRoom: "",
...
function render() {
  ...
  els.resultModal.classList.toggle("hidden", !["roundResult", "gameOver"].includes(room.status));
  ...
  if (room.status === "roundResult" && room.result) {
    ...
    els.choiceStatus.textContent = `${winner} 승리! 두 사람 모두 준비를 누르면 다음 라운드가 시작됩니다. (${readyCount}/2)`;
    renderRoundModal(room, parity, readyCount);
    playTransitionSound(room);
    return;
  }
...
async function leaveRoom() {
  ...
  resetToSetup();
  if (roomKey && isHost) await remove(roomRef(roomKey));
}
...
function playingNotice(room, myId) {
  ...
  return myId === room.guesserId
    ? `${hostName}님이 카드를 골랐습니다. 홀인지 짝인지 선택하세요.`
    : `${guesserName}님이 고민 중이에요. 선택이 끝나면 결과가 공개됩니다.`;
}
...
function renderFinalModal(isDraw, didWin, winner) {
  ...
  els.resultActionButton.textContent = "처음 화면으로";

  if (isDraw) {
    els.resultIcon.textContent = "⚡";
    els.resultModalTitle.textContent = "무승부!";
    els.resultModalMessage.textContent = "둘 다 끝까지 읽히지 않았어요. 다시 붙어야 하는 승부예요.";
    return;
  }
  if (didWin) {
    els.resultIcon.textContent = "🏆";
...
function renderRoundModal(room, parity, readyCount) {
  ...
  els.resultModalTitle.textContent = didWin ? "이번 판 가져왔습니다!" : "이번 판은 놓쳤습니다!";
  ...
  els.resultActionButton.disabled = Boolean(room.roundReady?.[state.playerId]);
  els.resultActionButton.textContent = room.roundReady?.[state.playerId]
    ? `상대 준비 대기 중 (${readyCount}/2)`
    : "다음 라운드 준비";
}
...
function toggleMute() {
  ...
  localStorage.setItem("holzzakMuted", state.muted ? "1" : "0");
...
function syncMuteButton() {
  els.muteButton.textContent = state.muted ? "🔇" : "🔊";
  els.muteButton.setAttribute("aria-label", state.muted ? "음소거 해제" : "음소거");
}
