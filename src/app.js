...
function render() {
  ...
  if (room.status === "waiting") {
    ...
    els.statusNotice.textContent = "친구에게 방 이름과 4자리 비밀번호를 알려주세요. 둘이 모이면 첫 라운드가 바로 시작됩니다.";
...
function startMusic() {
  ...
  state.musicTimer = setInterval(() => {
    ...
    playTone(melody[step % melody.length], now, 0.11, "triangle", 0.034);
    if (step % 2 === 0) playTone(130.81, now, 0.1, "sine", 0.02);
    step += 1;
  }, 250);
}
